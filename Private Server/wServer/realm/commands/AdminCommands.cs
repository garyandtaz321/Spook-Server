...
internal class TossEffCommand : Command
    ...
    {
        public SetGoldCommand() : base("gold", permLevel: 3) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
    ...
    {
        public SetTokensCommand() : base("tokens", permLevel: 3) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    internal class Ranktag : Command
    {
        public Ranktag()
            : base("ranktag", permLevel: 1)
        {
        }
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            {
                player.SendError("You are not staff on this server! Please use /tag");
                return false;
            }
            else
            {
                if (player.Client.Account.Rank == 1)
                {
                    player.Manager.Database.DoActionAsync(db =>
                    {
                        player.Name = "[" + "T-MOD" + "] " + player.Client.Account.Name;
                        player.UpdateCount++;
                    });
                    player.SendInfo("Your tag is now: [T-MODERATOR] " );
                }
                else if(player.Client.Account.Rank == 2){
                    player.Manager.Database.DoActionAsync(db =>
                    {
                        player.Name = "[" + "MODERATOR" + "] " + player.Client.Account.Name;
                        player.UpdateCount++;
                    });
                    player.SendInfo("Your tag is now: [MODERATOR] ");
                }
                else if (player.Client.Account.Rank == 3){
                    player.Manager.Database.DoActionAsync(db =>
                    {
                        player.Name = "[" + "T-ADMIN" + "] " + player.Client.Account.Name;
                        player.UpdateCount++;
                    });
                    player.SendInfo("Your tag is now: [T-ADMIN] ");
                }
                else if (player.Client.Account.Rank == 4){
                    player.Manager.Database.DoActionAsync(db =>
                    {
                        player.Name = "[" + "ADMIN" + "] " + player.Client.Account.Name;
                        player.UpdateCount++;
                    });
                    player.SendInfo("Your tag is now: [ADMIN] ");
                }
                else if (player.Client.Account.Rank == 5){
                    player.Manager.Database.DoActionAsync(db =>
                    {
                        player.Name = "[" + "OWNER" + "] " + player.Client.Account.Name;
                        player.UpdateCount++;
                    });
                    player.SendInfo("Your tag is now: [OWNER] ");
                }
            }
    ...
    {
        public SetFameCommand() : base("fame", permLevel: 3) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    internal class GiveCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            string[] BANNED = {
                "helm of the forgotten plague",
                "reaper key",
                "sword of malicious revenge",
                "blackmar gauntlet",
                "fear's battleguard",
                "bracer of the bot",
                "sword of the divine lord",
                "armor of the anti pomf pomf",
                "shield of clensing fury",
                "blackmar gauntlet",
                "shaitan's Key",
            };
            ...
            int rank = player.Client.Account.Rank;
            switch (rank)
            {
                case 0:
                    {

                        player.SendError("You do not have a rank on this server, if you believe this is an error, please contact a moderator!");
                        return false;
                        break;
                    }
                case 1:
                    {
                        ...
                        if (!TMOD.Contains(name.ToLower()))
                        {
                            player.SendError("Sorry! You cannot get this item due to your rank!, Logging this action to the server dev!");
                            log.FatalFormat(player.Name, "Tried to give himself a: ", name);
                ...
                case 2:
                    {
                        ...
                        if (MODTADMIN.Contains(name.ToLower()) || BANNED.Contains(name.ToLower()))
                        {
                            player.SendError("Sorry! You cannot get this item due to your rank!, Logging this action to the server dev!");
                            log.FatalFormat(player.Name, "Tried to give himself a: ", name);
                ...
                case 4:
                    {
                        ...
                        if (BANNED.Contains(name.ToLower()))
                        {
                            player.SendError("Sorry! You cannot get this item due to your rank!, Logging this action to the server dev!");
                            log.FatalFormat(player.Name, "Tried to give himself a: ", name);
    ...
    {
        public Maxplayer()
            : base("maxplayer", permLevel: 4) // Need to fix
        {
        }
        ...
        {
            {
                foreach (Client i in player.Manager.Clients.Values)
                {
                    if (i.Account.Name.EqualsIgnoreCase(args[0]))
                    {
                        try
                        {
                            i.Player.Client.Character.Level = 20;
                            i.Player.Client.Player.Level = 20;
                          

                            i.Player.Stats[0] = player.ObjectDesc.MaxHitPoints;
                            i.Player.Stats[1] = player.ObjectDesc.MaxMagicPoints;
                            i.Player.Stats[2] = player.ObjectDesc.MaxAttack;
                            i.Player.Stats[3] = player.ObjectDesc.MaxDefense;
                            i.Player.Stats[4] = player.ObjectDesc.MaxSpeed;
                            i.Player.Stats[5] = player.ObjectDesc.MaxHpRegen;
                            i.Player.Stats[6] = player.ObjectDesc.MaxMpRegen;
                            i.Player.Stats[7] = player.ObjectDesc.MaxDexterity;
    ...
    internal class SWhoCommand : Command //get all players from all worlds (this may become too large!) Pot Server crasher -- will fix
    {
        public SWhoCommand()
            : base("swho", permLevel: 5)
        {
        }
